...
- (void)viewDidLoad {
    ...
    self.timerOne = [NSTimer scheduledTimerWithTimeInterval:1.0 repeats:YES block:^(NSTimer * _Nonnull timer) {
        // title label
        self.showTitleLabel.text = [NSDate stringNormalReadWithDate:[NSDate date]];
        
        // second Label
        self.showTwoLabel.text = [NSString stringWithFormat:@"%.0f", [NSDate timeIntervalSinceReferenceDate]];
        
        // third Label
        self.showThreeLabel.text = [NSDate stringFromDate:[NSDate date] andFormatterString:@"yyyy-MM-dd HH:mm:ss"];
    }];
    
    // timer two
    self.timerTwo = [NSTimer scheduledTimerWithTimeInterval:0.05 repeats:YES block:^(NSTimer * _Nonnull timer) {
        // first Label
        NSArray *currentDateInfoArray = [NSDate getDateYearMonthDayWithDate:[NSDate date]];
